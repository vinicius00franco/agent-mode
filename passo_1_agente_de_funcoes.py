...
async def main():
    ...
    ferramenta_calculo = config.FunctionTool.from_defaults(fn=config.calcular_engajamento)
    ferramenta_consulta_arxiv = config.FunctionTool.from_defaults(fn=config.consulta_artigos)
    ...
    response2 = await agent.run("Me retorne artigos sobre o uso da inteligência artificial nas redes sociais")
